...
export default async function IntegrationsPage({ searchParams }: { searchParams: Promise<{ tiktok?: string }> }) {
  ...
  const connectedCount = Number(status.connected) + Number(shopeeEnabled);
  ...
  return <main className="mx-auto w-full max-w-[1600px] px-4 py-5 sm:px-6 lg:px-5">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={Link2} tone="bg-[#e9f8de] text-[#64bd27]" label="Sàn đang kết nối" value={String(connectedCount)} hint={connectedCount ? "Đang có kênh affiliate hoạt động" : "Chưa có sàn nào được cấu hình"} /><Metric icon={ShieldCheck} tone="bg-[#e8f1ff] text-[#2b78e9]" label="Kết nối hoạt động ổn định" value={`${connectedCount}/2`} hint={shopeeEnabled ? "Shopee Data API đã được cấu hình" : status.connected ? "TikTok token đang có hiệu lực" : "Chờ cấu hình thông tin sàn"} /><Metric icon={Clock3} tone="bg-[#f4e8ff] text-[#a142db]" label="Đồng bộ đơn hàng" value={status.connected ? "TikTok sẵn sàng" : "—"} hint="Data API Shopee không cung cấp conversion report" /><Metric icon={ShoppingBag} tone="bg-[#fff3dc] text-[#eda815]" label="Đơn đã đồng bộ hôm nay" value="—" hint="Đơn Shopee cần được đối soát thủ công" /></section>
    <section className="mt-3 grid gap-3 xl:grid-cols-2"><article className="overflow-hidden rounded-xl border border-[#e4ebf5] bg-white"><div className="flex items-center justify-between border-b border-[#edf1f7] px-5 py-4"><div className="flex items-center gap-3"><MarketplaceLogo platform="shopee" /><div><h2 className="font-black text-[#102e5c]">Shopee</h2><p className="text-[11px] text-[#6c86a8]">Data API (AddliveTag)</p></div></div><Badge variant={shopeeEnabled ? "success" : "warning"} className="px-3 py-1 text-[11px]">{shopeeEnabled ? "Đang hoạt động" : shopeeConfigured ? "Chưa bật provider" : "Chưa cấu hình"}</Badge></div><div className="p-5"><p className="text-sm leading-6 text-[#60799c]">Dùng Data API không chính thống để kiểm tra sản phẩm, ước tính hoa hồng và tạo link <code className="rounded bg-[#f1f5fb] px-1.5 py-0.5 text-xs text-[#35557e]">an_redir</code> từ Affiliate ID. Không cần Shopee App Secret.</p><div className={`mt-5 flex items-center gap-2 rounded-lg p-3 text-xs ${shopeeEnabled ? "bg-[#f0faed] text-[#426448]" : "bg-[#f8fbff] text-[#587298]"}`}>{shopeeEnabled ? <CircleCheck className="size-4 text-[#26943d]" /> : <CircleAlert className="size-4 text-[#e9a414]" />}{shopeeEnabled ? "Đang dùng AddliveTag Data API." : shopeeConfigured ? "Thêm AFFILIATE_PROVIDER_SHOPEE=addlivetag để kích hoạt link thật." : "Thiết lập ADDLIVETAG_API_KEY và SHOPEE_AFFILIATE_ID trong môi trường triển khai."}</div></div></article><article className="overflow-hidden rounded-xl border border-[#e4ebf5] bg-white"><div className="flex items-center justify-between border-b border-[#edf1f7] px-5 py-4"><div className="flex items-center gap-3"><MarketplaceLogo platform="tiktok" /><div><h2 className="font-black text-[#102e5c]">TikTok Shop</h2><p className="text-[11px] text-[#6c86a8]">Affiliate Creator API</p></div></div><Badge variant={status.connected ? "success" : "warning"} className="px-3 py-1 text-[11px]">{status.connected ? "Đang hoạt động" : "Chưa kết nối"}</Badge></div><div className="p-5"><TikTokIntegration status={status} /></div></article></section>
    <section className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1.65fr)_minmax(360px,1fr)]"><article className="rounded-xl border border-[#e4ebf5] bg-white"><h2 className="border-b border-[#edf1f7] px-5 py-4 text-sm font-black text-[#12355f]">Trạng thái hệ thống kết nối</h2><div className="divide-y divide-[#edf1f7] px-5"><div className="flex items-center gap-3 py-4"><MarketplaceLogo platform="shopee" size={32} /><div className="flex-1"><b className="text-sm text-[#35557e]">Shopee</b><p className="text-[11px] text-[#8298b6]">{shopeeEnabled ? "Tạo link qua Data API; đối soát đơn thủ công" : shopeeConfigured ? "Đã có API Key và Affiliate ID, chưa bật provider" : "Chờ API Key và Affiliate ID"}</p></div><span className={`flex items-center gap-1 text-xs font-bold ${shopeeEnabled ? "text-[#26943d]" : "text-[#d88700]"}`}>{shopeeEnabled ? <CircleCheck className="size-4" /> : <CircleAlert className="size-4" />}{shopeeEnabled ? "Hoạt động" : "Cần thiết lập"}</span></div><div className="flex items-center gap-3 py-4"><MarketplaceLogo platform="tiktok" size={32} /><div className="flex-1"><b className="text-sm text-[#35557e]">TikTok Shop</b><p className="text-[11px] text-[#8298b6]">{status.connected ? status.sellerName ?? "Creator đã kết nối" : "Chờ uỷ quyền Creator"}</p></div><span className={`flex items-center gap-1 text-xs font-bold ${status.connected ? "text-[#26943d]" : "text-[#d88700]"}`}>{status.connected ? <CircleCheck className="size-4" /> : <CircleAlert className="size-4" />}{status.connected ? "Hoạt động" : "Chưa kết nối"}</span></div></div></article><article className="rounded-xl border border-[#e4ebf5] bg-white"><h2 className="border-b border-[#edf1f7] px-5 py-4 text-sm font-black text-[#12355f]">Cấu hình hoa hồng theo sàn</h2><div className="space-y-3 p-5 text-sm"><p className="flex justify-between text-[#577298]"><span>Shopee</span><b className={shopeeEnabled ? "text-[#2f7a1c]" : "text-[#8298b6]"}>{shopeeEnabled ? "Ước tính từ Data API" : "Chưa thiết lập"}</b></p><p className="flex justify-between text-[#577298]"><span>TikTok Shop</span><b className="text-[#102e5c]">Theo hoa hồng Affiliate</b></p><p className="border-t border-[#edf1f7] pt-3 text-[11px] text-[#8298b6]">Tỷ lệ hoàn tiền cho người dùng được tính từ cấu hình CASHBACK_RATE hiện tại.</p></div></article></section>
  </main>;
}
